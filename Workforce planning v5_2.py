...
def calculate_month_buckets(year, month, volume, current_fte, config, graduates=0, trainees_in_pipeline=0):
    ...
    for bucket_name, bucket_def in bucket_defs.items():
        ...
        if day_hours_month > 0 and night_hours_month > 0:
            n_available = min(concurrent_agents_from_day, concurrent_agents_from_night)
            binding_shift = 'day' if concurrent_agents_from_day <= concurrent_agents_from_night else 'night'
        elif day_hours_month > 0:
            n_available = concurrent_agents_from_day
            binding_shift = 'day'
        elif night_hours_month > 0:
            n_available = concurrent_agents_from_night
            binding_shift = 'night'
        else:
            n_available = 0
            binding_shift = 'none'
        ...
        concurrent_agent_gap = n_available - n_required
        ...
        # Each pool needs to independently provide n_required concurrent agents during its hours
...
for r in test_results:
    print(f"{r['bucket']:<20} {r['volume']:>8,} {r['total_hours']:>6} {r['calls_per_hour']:>9.1f} {r['erlangs']:>7.1f} {r['n_required']:>6} {r['n_available']:>7} {r['n_deployed']:>6} {r['concurrent_agent_gap']:>+5} {r['fte_required']:>8.1f} {r['achievable_sla']:>5.0%} {r['agent_occupancy']:>5.0%} {r['zone']:<12}")
