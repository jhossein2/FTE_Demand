...
CONFIG = {
    ...
    'min_concurrent_agents_floor': 12,       # Union minimum
...
def calculate_month_buckets(year, month, volume, current_fte, config, graduates=0, trainees_in_pipeline=0):
    ...
    for bucket_name, bucket_def in bucket_defs.items():
        ...
        # Available concurrent agents from each shift
        concurrent_agents_from_day = int(effective_fte_day / roster_mult_day) if roster_mult_day > 0 else 0
        concurrent_agents_from_night = int(effective_fte_night / roster_mult_night) if roster_mult_night > 0 else 0
        ...
        if day_hours_month > 0 and night_hours_month > 0:
            concurrent_agents_available = min(concurrent_agents_from_day, concurrent_agents_from_night)
            binding_shift = 'day' if concurrent_agents_from_day <= concurrent_agents_from_night else 'night'
        elif day_hours_month > 0:
            concurrent_agents_available = concurrent_agents_from_day
            binding_shift = 'day'
        elif night_hours_month > 0:
            concurrent_agents_available = concurrent_agents_from_night
            binding_shift = 'night'
        else:
            concurrent_agents_available = 0
            binding_shift = 'none'
        ...
        agents_floor = config['min_concurrent_agents_floor']
        
        # required_agents = max of all constraints
        required_agents = max(agents_for_sla, agents_for_occupancy, agents_floor)
        
        # PHASE 4: Performance Audit (The Result)
        
        # Floor is HARD constraint - we MUST deploy at least floor
        # Only if concurrent_agents_available >= floor, we can meet it
        # If concurrent_agents_available < floor, we still deploy floor (overtime/borrowed staff) but flag breach
        floor_breach = concurrent_agents_available < agents_floor  # Can't meet floor with available capacity
        
        # N_deployed: enforce floor as minimum, cap at available (but at least floor)
        if concurrent_agents_available >= agents_floor:
            # Normal case: deploy what's needed, capped by available
            n_deployed = min(required_agents, concurrent_agents_available)
        else:
        ...
        if concurrent_agents_available > 0:
            capacity_utilization = n_deployed / concurrent_agents_available
        else:
            capacity_utilization = 1.0
        
        # Concurrent agent gap (positive = surplus, negative = deficit)
        concurrent_agent_gap = concurrent_agents_available - required_agents
        ...
        # Each pool needs to independently provide required_agents concurrent agents during its hours
        fte_required_day = required_agents * roster_mult_day if day_hours_month > 0 else 0
        fte_required_night = required_agents * roster_mult_night if night_hours_month > 0 else 0
        ...
        results.append({
            ...
            'concurrent_agents_available': concurrent_agents_available,
            'concurrent_agents_from_day': concurrent_agents_from_day,
            'concurrent_agents_from_night': concurrent_agents_from_night,
            'binding_shift': binding_shift,
            'n_deployed': n_deployed,
            'concurrent_agent_gap': concurrent_agent_gap,
...
for r in test_results:
    print(f"{r['bucket']:<20} {r['volume']:>8,} {r['total_hours']:>6} {r['calls_per_hour']:>9.1f} {r['erlangs']:>7.1f} {r['required_agents']:>6} {r['concurrent_agents_available']:>7} {r['n_deployed']:>6} {r['concurrent_agent_gap']:>+5} {r['fte_required']:>8.1f} {r['achievable_sla']:>5.0%} {r['agent_occupancy']:>5.0%} {r['zone']:<12}")
...
columns = [
    ...
    'agents_floor', 'required_agents', 'concurrent_agents_available', 
    'concurrent_agents_from_day', 'concurrent_agents_from_night', 'binding_shift',
    'n_deployed', 'concurrent_agent_gap',
...
if len(floor_breaches) > 0:
    ...
    print(f"   Floor required: {first_floor['agents_floor']} | Available capacity: {first_floor['concurrent_agents_available']}")
...
if len(sla_breaches) > 0:
    ...
    print(f"   Required: {first_breach['required_agents']} | Available: {first_breach['concurrent_agents_available']} | Deployed: {first_breach['n_deployed']}")
...
def create_monthly_summary(df):
    ...
    - required_agents: Max across buckets
    - concurrent_agents_available: Min across buckets (binding constraint)
    ...
    for (year, month), group in df.groupby(['year', 'month']):
        ...
        # Find binding bucket (lowest concurrent_agents_available)
        binding_idx = group['concurrent_agents_available'].idxmin()
        ...
        monthly_rows.append({
            ...
            # Supply
            'required_agents': group['required_agents'].max(),
            'concurrent_agents_available': group['concurrent_agents_available'].min(),
...
display_cols = [
    ...
    'required_agents', 'concurrent_agents_available', 'min_sla', 'max_occupancy', 
